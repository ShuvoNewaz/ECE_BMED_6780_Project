...
from src.models.esfpnet import mit
from src.models.esfpnet import mlp
from mmcv.cnn import ConvModule
from src.training.metrics import ange_structure_loss


pretrain_path = './pretrained/esfpnet'

class ESFPNetStructure(nn.Module):

    def __init__(self, model_type: str, embedding_dim: int, dropout=0.1):
        super().__init__()
    ...
    def _init_weights(self):
        ...
        if self.model_type == 'B0':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b0.pth'))
        if self.model_type == 'B1':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b1.pth'))
        if self.model_type == 'B2':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b2.pth'))
        if self.model_type == 'B3':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b3.pth'))
        if self.model_type == 'B4':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b4.pth'))
        if self.model_type == 'B5':
            pretrained_dict = torch.load(os.path.join(pretrain_path, 'mit_b5.pth'))
    ...
    def forward(self, x):
        ...
        out_resized = torch.squeeze(out_resized, dim=1)
