...
def IOU(prediction, target):
    ...
    iou_score = iou_score.float()

    return iou_score.nan_to_num(0)


def BinaryF1Stage1(prediction, target):
...
def BinaryF1(prediction, target):
    TP = (prediction[target == 1] == 1).sum()
    TN = (prediction[target == 0] == 0).sum()
    FP = (prediction[target == 0] == 1).sum()
    FN = (prediction[target == 1] == 0).sum()

    accuracy = ((TP + TN) / (TP + TN + FP + FN)).nan_to_num(0)
    specificity = (TN / (TN + FP)).nan_to_num(0)
    precision = (TP / (TP + FP)).nan_to_num(0)
    recall = (TP / (TP + FN)).nan_to_num(0)
...
def dice_loss(logits, target):
    ...
    dice_score = BinaryF1(prediction, target)
